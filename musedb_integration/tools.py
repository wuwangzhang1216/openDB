"""Tool factory: create MuseDB-backed tools compatible with any host tool system.
...
# Binary document formats that MuseDB handles better than local extraction
_DOCUMENT_EXTENSIONS = {".pdf", ".docx", ".pptx", ".xlsx", ".csv", ".xls", ".doc"}

# Image extensions (MuseDB has OCR)
...
def create_tools(client: MuseDBClient, ToolBase: type, ResultClass: type) -> list:
    """Create 3 MuseDB-backed tool instances that replace read/grep/glob.

    The returned tools have IDs "read", "grep", "glob" — directly replacing
    the host app's built-in tools. Each tool tries MuseDB first and falls back
    to local logic when MuseDB is unavailable.

    Args:
        client: MuseDBClient instance.
    ...
    class MuseDBReadTool(ToolBase):
        ...
        async def execute(self, args: dict[str, Any], ctx: Any) -> Any:
            ...
            # Build MuseDB lines param from offset/limit
            lines_param = None
    ...
    class MuseDBSearchTool(ToolBase):
    ...
    return [MuseDBReadTool(), MuseDBSearchTool(), MuseDBGlobTool()]
...
def _format_search_results(data: dict, pattern: str, ResultClass: type) -> Any:
    """Format MuseDB search results into tool output."""
